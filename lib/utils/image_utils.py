...
def vertical_crop_image(image, top_offset, bottom_offset):
    """
    Remove a top & button amount of positions. i.e. given an (100, 100, 3) utils and applies:

    vertical_crop(utils, 30, 30)
...
def resize_image(image, width, height):
    """
    Resize the utils to the input shape used by the network model
    """
...
def rgb_to_yuv(image):
    """
    Convert the utils from RGB to YUV (This is what the NVIDIA model does)
    """
