...
from lib.utils.plot_utils import histograms
...
class Dataset:
    ...
    def show_labels_stats(self):
        ...
        titles = [f'Range: ({self.label_column(i).min():0.4f} , {self.label_column(i).max():0.4f})' for i in
                  range(len(self.label_columns))]
        histograms(values, x_labels, titles, vertical=True)
